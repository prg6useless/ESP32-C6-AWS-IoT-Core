...
#define CONFIG_AWS_EXAMPLE_CLIENT_ID AWS_IOT_MQTT_CLIENT_ID
...
void aws_iot_task(void *param)
{
    ...
    const char *TOPIC = "esp32/subscribe";
    // const char *TOPICs = "topic/esp32/publish/";
    const int TOPIC_LEN = strlen(TOPIC);
    // const int TOPICs_LEN = strlen(TOPICs);
    ...
    sprintf(cPayload, "%s : %d ", "hello from SDK c6", i);
    ...
    while ((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc))
    {
        ...
        vTaskDelay(1000 / portTICK_PERIOD_MS);
        sprintf(cPayload, "%s : %d ", "hello from ESP32-c6s (QOS0)", i++);
        paramsQOS0.payloadLen = strlen(cPayload);
        rc = aws_iot_mqtt_publish(&client, TOPIC, TOPIC_LEN, &paramsQOS0);

        sprintf(cPayload, "%s : %d ", "hello from ESP32-c6s (QOS1)", i++);
...
void app_main()
{
    ...
    xTaskCreatePinnedToCore(&aws_iot_task, "aws_iot_task", 9216, NULL, 5, NULL, 0); // Pin to Core 0
}
